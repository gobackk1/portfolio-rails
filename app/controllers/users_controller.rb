class UsersController < ApplicationController
  ...
  def report
    ...
    to = (Time.now - 6.day).at_end_of_day
    current_user_records = @current_user.study_records
    weekly_records = current_user_records.where(created_at: 1.week.ago.at_beginning_of_day..Time.now.at_end_of_day)

    study_time_per_week = []
    (0..6).to_a.map do |i|
      study_time_per_week << {
        date: I18n.l(Time.now - (6 - i).days, format: "%m月%d日"),
        study_hour: weekly_records.where(created_at: (6 - i).day.ago.all_day).sum(:study_hours)
      }
    end

    teaching_materials = weekly_records.select(:teaching_material_name).distinct.map do |material|
      material.teaching_material_name
    end
    report_by_teaching_material =[]
    teaching_materials.map do |material|
      study_hours = weekly_records.where(teaching_material_name: material).sum(:study_hours)
